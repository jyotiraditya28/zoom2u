        ...
        viewModel.getEditLocationSuccess()?.observe(this) {
            ...
            viewModel.getDataFromGoogleSuccess()?.observe(this) {its->
                if (!TextUtils.isEmpty(its)) {
                    AppUtility.progressBarDissMiss()
                    if (its != "") {
                        viewModel.getIsForEdit()?.observe(this) {
                            if (it != null) {
                                ...
                                if (it) {
                                    ...
                                    myLocationResponse?.Location?.GPSX =
                                        resultsJsonArray.getJSONObject(0).getJSONObject("geometry")
                                    ...
                                    addLocationReq?.Location?.Gpsx =
                                        resultsJsonArray.getJSONObject(0).getJSONObject("geometry")
                                            .getJSONObject("location").getDouble("lat")
                                    addLocationReq?.Location?.Gpsy =
                                        resultsJsonArray.getJSONObject(0).getJSONObject("geometry")
                                            .getJSONObject("location").getDouble("lng")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == placeAutocompleteRequest) {
            if (resultCode == RESULT_OK) {
                ...
                viewModel.dataFromGoogle(binding.address.text.toString(),isEdit)
                ...
                Log.i("Place API Failure", "  -------------- Error -------------" + status.statusMessage
                )
